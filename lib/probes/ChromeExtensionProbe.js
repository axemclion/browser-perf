...
ChromeExtensionProbe.prototype.teardown = function(browser) {
	...
	return browser.sessionCapabilities().then(function(caps) {
		if (caps.browserName.match(/chrome/gi)) {
			return me.fetchTimelineData(browser).then(function() {
				if (!caps.platform.match(/android/gi)) {
...
ChromeExtensionProbe.prototype.fetchTimelineData = function(browser) {
	var me = this,
		windowName = null;
...
ChromeExtensionProbe.prototype.fetchTracingData = function(browser) {
	var me = this;
	log.debug('[ChromeExtensionProbe]: Fetching Tracing Data');
