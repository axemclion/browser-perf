	...
	for (var i = 0; i < this.data.length; i++) {
		for (var j = 0; j < this.data[i].length; j++) {
			...
			if (val.name === 'ImplThreadRenderingStats::IssueTraceEvent' || val.name === 'MainThreadRenderingStats::IssueTraceEvent') {
				// This one was till Chrome 31
				result.push(val);
			}
			if (val.name === 'BenchmarkInstrumentation::ImplThreadRenderingStats' || val.name === 'BenchmarkInstrumentation::MainThreadRenderingStats') {
				// Chrome 33 has new names for these
				result.push(val);
			}
		}
	}
