...
function main(url, cb, cfg) {
	...
	opts.browsers.map(function(browserConfig) {
		return runOnBrowser(url, browserConfig, opts);
	}).reduce(Q.when, Q());
}
...
function runOnBrowser(url, browserConfig, opts) {
	...
	metrics.setup(opts).then(function() {
		return browser.init(browserConfig);
	}).then(function() {
		return browser.get(url);
	}).then(function() {
		return metrics.start(opts, browser);
	}).then(function() {
		return actions.perform(browser, opts.actions);
	}).then(function() {
		return metrics.teardown(opts, browser);
	}).then(function() {
		return metrics.getResults();
	}).then(function(data) {
		console.log(helpers.metrics.log(data));
	}).fin(function() {
		return browser.quit();
	}).done(function() {
