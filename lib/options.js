...
var defaults = function(opts) {
	...
	opts = parseCredentials(opts);
	opts = parsePreScript(opts);

	validateOptions(opts);
	return opts;
};

var parsePreScript = function(opts) {
	if (opts.preScriptFile) {
		var path = require('path');
		opts.preScript = require(path.resolve(opts.preScriptFile));
	}

	opts.preScript = opts.preScript || function(browser, cb) {
		cb(null, true);
	};
	return opts;
}
...
var parseCredentials = function(opts) {
	if (opts.seleniumConfig.hostname.match(/ondemand.saucelabs.com/)) {
		opts.SAUCE_USERNAME = opts.username;
		opts.SAUCE_ACCESSKEY = opts.accesskey;
	} else if (opts.seleniumConfig.hostname.match(/ondemand.saucelabs.com/)) {
		opts.BROWSERSTACK_USERNAME = opts.username;
		opts.BROWSERSTACK_KEY = opts.key;
	}

	if (typeof opts.BROWSERSTACK_USERNAME !== 'undefined') {
		opts.browserConfig.forEach(function(browser) {
			browser['browserstack.user'] = opts.BROWSERSTACK_USERNAME;
			browser['browserstack.key'] = opts.BROWSERSTACK_KEY;
		});
	} else if (typeof opts.SAUCE_USERNAME !== 'undefined') {
		opts.seleniumConfig.username = opts.SAUCE_USERNAME;
		opts.seleniumConfig.pwd = opts.SAUCE_ACCESSKEY;
	}
	return opts;
}
