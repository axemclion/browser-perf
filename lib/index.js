var wd = require('wd'),
	fs = require('fs'),
	bunyan = require('bunyan'),
	jsmin = require('jsmin').jsmin,
	glob = require('glob').sync,
	FirefoxProfile = require('firefox-profile');

var SmoothnessMetric = require('./metrics/smoothness');

var log = null;

var telemetryScript = function() {
	var scripts = [];
	[
		'vendor/scroll.js',
		'vendor/smoothness.js',
		'benchmarks.js',
		'runner.js'
	].forEach(function(file) {
		scripts.push(jsmin(fs.readFileSync(__dirname + '/scripts/' + file, {
			encoding: 'utf-8'
		})).replace(/\'use strict\'/, ''));
	});
	return scripts.join(';');
}

var processFrameData = function(result) {
	if (typeof result.frame_data !== 'undefined') {
		log.debug('Result has frame data, processing it now');
		var smoothness = new SmoothnessMetric(result.frame_data);
		smoothness.addData(result);
		delete result.frame_data;
		log.debug('Processing frame data complete');
	}
	return result;
}

var testRunner = function(browser, url, config, cb) {
	var result;
	log.debug('Running on', config.browserName);
	browser
		.init(config)
		.get(url)
		.execute(telemetryScript())
		.waitFor({
			asserter: wd.asserters.jsCondition('(typeof window.__telemetryData__ !== "undefined")', false),
			timeout: 1000 * 60 * 10,
			pollFreq: 100
		})
		.eval('window.__telemetryData__')
		.then(function(data) {
			log.debug('Got data back');
			result = data;
		})
		.fin(function() {
			log.debug('Completed running on', config.browserName);
			if (!config.debug) {
				return browser.quit();
			}
		})
		.then(function() {
			log.debug('Returning results');
			processFrameData(result);
			cb(null, result);
		}, function(err) {
			log.debug('An error occured, returning error');
			cb(err);
		});
}

var defaults = function(opts) {
	opts = opts || {};
	log = opts.logger || bunyan.createLogger({
		name: 'index.js',
		level: 'debug',
		stream: process.stdout
	});
	opts.selenium = opts.selenium || {
		hostname: 'ondemand.saucelabs.com',
		port: 80,
		user: process.env.SAUCE_USERNAME,
		pwd: process.env.SAUCE_ACCESS_KEY,
	};
	opts.browsers = parseBrowsers(opts.browsers || opts.browser);
	return opts;
};

var parseBrowsers = function(browsers) {
	if (typeof browsers === 'undefined') {
		browsers = ['chrome'];
	} else if (typeof browsers === 'String') {
		browsers = [browsers];
	}

	var browserConfig =
		[{
		browserName: 'chrome',
		chromeOptions: {
			args: ['--disable-popup-blocking', '--enable-gpu-benchmarking', '--enable-thread-composting'],
			extensions: glob(__dirname + '/**/*.crx').map(function(file) {
				return fs.readFileSync(file).toString('base64');
			})
		},
		'disable-popup-handler': true
	}, {
		browserName: 'firefox',
		firefox_profile: (function() {
			var fp = new FirefoxProfile();
			fp.setPreference('dom.send_after_paint_to_content', true);
			fp.setPreference('dom.disable_open_during_load', false);
			fp.updatePreferences();
			return fp.encodedSync();
		}()),
		'disable-popup-handler': true
	}],
		result = [];

	browsers.forEach(function(browserName) {
		var browser = (typeof browserName === 'string' ? {
			'browserName': browserName
		} : browserName);

		var resultCfg = browserConfig.filter(function(cfg) {
			if (cfg.browserName !== browser.browserName) {
				return false;
			} else {
				for (var key in browser) {
					cfg[key] = browser[key];
				}
				return cfg;
			}
		});

		if (resultCfg.length === 0) {
			resultCfg = [browser];
		}
		var config = resultCfg[0];

		// Extension loading for Chrome
		if (config.browserName === 'chrome' && config.debug === true) {
			config.chromeOptions.args.push('--load-extension=' + __dirname + '/scripts');
			delete config.chromeOptions.extensions;
		}
		result.push(config);
	});
	log.trace(result);
	return result;
};

module.exports = function(url, cb, opts) {
	opts = defaults(opts);
	var browser = wd.promiseChainRemote(opts.selenium),
		results = [],
		errors = [];

	browser.on('status', function(data) {
		log.debug(data);
	});
	browser.on('command', function(meth, path, data) {
		if (data && typeof data === 'object') {
			var str = JSON.stringify(data);
			if (str.length > 80)
				data = str.substring(0, 75) + '...';
		}
		log.debug(' > ', meth, path, '');
	});

	(function runTest(i) {
		if (i < opts.browsers.length) {
			testRunner(browser, url, opts.browsers[i], function(err, res) {
				if (err) {
					errors.push(err);
				} else {
					results.push(res);
				}
				runTest(i + 1);
			});
		} else {
			cb(errors.length === 0 ? null : errors, results);
		}
	}(0));
}