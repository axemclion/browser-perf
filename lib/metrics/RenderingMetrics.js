...
RenderingMetrics.prototype.getResults = function() {
	...
	for (var key in this.timelineMetrics) {
		res[key] = helpers.metrics(this.timelineMetrics[key].sum, 'RenderingMetrics', 'ms', 0);
		res[key + '_avg'] = helpers.metrics(this.timelineMetrics[key].sum / this.timelineMetrics[key].count, 'RenderingMetrics', 'ms', 2);
		res[key + '_max'] = helpers.metrics(this.timelineMetrics[key].max, 'RenderingMetrics', 'ms', 1);
		res[key + '_count'] = helpers.metrics(this.timelineMetrics[key].count, 'RenderingMetrics', 'count', 1);
	}
